        ...
        <div className="hidden sm:flex items-center gap-4 text-sm">
          {user ? (
            <>
              ...
              {user.isAdmin && <Link to="/admin" className="hover:text-yellow-400">Admin Panel</Link>}
              <span className="text-gray-300">Olá, <strong>{user.username || user.email}</strong></span>
              {user.isAdmin && <span className="text-xs bg-yellow-400 text-black px-2 py-0.5 rounded-full">Admin</span>}
              <button
        ...
        <div className="sm:hidden mt-4 space-y-2 text-sm px-4">
          {user ? (
            <>
              ...
              {user.isAdmin && <Link to="/admin" className="block hover:text-yellow-400">Admin Panel</Link>}
